...
function getWebViewContent(context, templatePath) {
    ...
    html = html.replace("cur: '0.0'", "cur: 'CSR'");
...
function invokeCallback(panel, message, resp) {
    ...
    if (message.cmd != "getText")
        panel.webview.postMessage({ cmd: 'vscodeCallback', cbid: message.cbid, data: resp });
    else{
        console.log("webview postmessage: sendText:"+ resp);
        panel.webview.postMessage({ cmd: 'sendText', cbid: message.cbid, data: resp });
    }
}
...
const messageHandler = {
    ...
    getText(global, message, text) {
        console.log("getting full text:" + text);
...
module.exports = function(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('extension.cswebview', function(uri) {
        console.log(uri);
        ...
        //return;
        //}
        var text = util.getText();
        var wsuri = "ws://localhost:8080/CRServer/websocket";
        if(panel){
            ...
            panel.webview.html = getWebViewContent(context, 'src/view/csrecommendation.html');
        }
        ...
        panel.webview.onDidReceiveMessage(message => {
            if (messageHandler[message.cmd]) {
                if(message.cmd == "getText"){
                    messageHandler[message.cmd](global, message, text);
                }
                else
